package com.etpa.energy.management.config;

import com.etpa.energy.management.config.constant.ErrorCodes;
...
public class GlobalExceptionHandler {
    ...
    protected WrapperResponse handleBusinessException(BusinessException ex, WebRequest request) {
        ...
        var errorMessages = messageSource.getMessage(Objects.requireNonNull(ex.getMessage()),null, request.getLocale());
    ...
    @ResponseStatus(value = HttpStatus.BAD_REQUEST)
    protected WrapperResponse handleConstraintViolationException(ConstraintViolationException ex, WebRequest request) {

        var errorMessages = ex.getConstraintViolations()
            .stream()
            .map(message -> message.getPropertyPath() +  " " + messageSource.getMessage(Objects.requireNonNull(message.getMessage()), null, request.getLocale()))
            .collect(Collectors.toList());
